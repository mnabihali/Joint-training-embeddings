...
cp_path = '/data/mohamed/jt/code/wav2vec_large.pt'
model_emb, cfg, task = fairseq.checkpoint_utils.load_model_ensemble_and_task([cp_path])
model_emb = model_emb[0]
model_emb.eval()
model_emb = model_emb.to(device2)
...
def emb(x):
    x = x.squeeze(dim=0)
    z = model_emb.feature_extractor(x)
    emb = model_emb.feature_aggregator(z)
    return emb
